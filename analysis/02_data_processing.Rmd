...
```{r, message=FALSE}
duke_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_Duke.xlsx"), sheet = 2)
```
...
```{r, message=FALSE}
ecnu_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_ECNU_NEW.xlsx"), sheet = 2)
```
...
```{r, message=FALSE}
fandm_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_FandM.xlsx"), sheet = 2)
```
...
```{r, message=FALSE}
lrc_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_LRC.xlsx"), sheet = 2)
```
...
Performance data not included yet.

## TBRI

```{r, message=FALSE}
tbri_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_TBRI.xlsx"), sheet = 2)
```
...
tbri <- tbri_raw %>%
  filter(pick!=("n/a"))%>%
  mutate(pick=as.numeric(pick), correct=as.numeric(correct))%>%
  mutate(species = gsub(" ", "_", species))
```
...
```{r, message=FALSE}
doue_la_fontaine_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_template_Doué-la-Fontaine.xlsx"), sheet = 2)
```

```{r}
do_checks(doue_la_fontaine_raw)
```

```{r}
# change data file
doue_la_fontaine <- doue_la_fontaine_raw
```
...
```{r, message=FALSE}
vienna_zoo_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_Vienna_Zoo.xlsx"), sheet = 2)
```

```{r}
do_checks(vienna_zoo_raw)
```

```{r}
# change data file
vienna_zoo <- vienna_zoo_raw %>%
  filter(!is.na(pick))%>%
  mutate(species = gsub(" ", "_", species))
```
...
```{r, message=FALSE}
wkprc_raw <- read_excel(str_c(path, "ManyPrimates_mp1_datasheet_WKPRC.xlsx"), sheet = 2)
```
