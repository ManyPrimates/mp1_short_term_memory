...
if(file.exists(factor_delay_model_filename)) {
    ...
    data$factor_delay <- as.factor(data$delay)
    factor_delay_model <- brm(correct ~ factor_delay +
                      ...
                      cores= 4,
                      prior = prior(skew_normal(0, 1.5, 3), class=Intercept) +
                              prior(skew_normal(-1.4, 2, 3), class=b, coef=factor_delay0) +
                              ...
                              prior(normal(0, 1), class=sd, coef=Intercept, group=site) +
                              prior(normal(0, 1), class=sd, coef=Intercept, group=species) +
                              prior(normal(0, 0.5), class=sd, coef=factor_delay0, group=subject_site) +
                              prior(normal(0, 0.5), class=sd, coef=factor_delay0, group=site) +
                              prior(normal(0, 0.5), class=sd, coef=factor_delay0, group=species) +
...
if(file.exists(model_comparison_filename)) {
    ...
    comp_waic <-  loo_compare(numeric_delay_model, null_model, criterion = c("waic")) %>%
                      as_tibble(rownames = "model")

    comp_loo <-  loo_compare(numeric_delay_model, null_model, criterion = c("loo")) %>%
                      as_tibble(rownames = "model")

    weights_waic <- model_weights(numeric_delay_model, null_model, weights = "waic") %>%
        as_tibble() %>%
        rename(waic_weight = value) %>%
        mutate(model  = c("numeric_delay_model", "null_model"),
            waic_weight = waic_weight %>% round(digits = 4)) %>%
        select(model, waic_weight)

    weights_loo <- model_weights(numeric_delay_model, null_model, weights = "loo")%>%
        as_tibble() %>%
