...
```{r}
full_model_filename <- "../saves/conf_full_model.rds"
if(file.exists(full_model_filename)) {
    full_model <- readRDS(full_model_filename)
} else {
    ...
    saveRDS(full_model, full_model_filename)
}
```
...
if(file.exists(model_comparison_filename)) {
    ...
    comp_waic <-  loo_compare(full_model, null_model, criterion = c("waic")) %>%
                      as_tibble(rownames = "model")

    comp_loo <-  loo_compare(full_model, null_model, criterion = c("loo")) %>%
                      as_tibble(rownames = "model")
