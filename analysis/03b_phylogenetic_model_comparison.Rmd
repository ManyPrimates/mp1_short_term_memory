...
model_pred <-  bind_rows(
  ...
  dietary_breadth_model%>% 
  posterior_samples(pars = c("b_Intercept","b_delay","b_dietary_breadth","b_delay:dietary_breadth"), subset = runif(1000, 0, 20000))%>%
  rownames_to_column(var = "iteration")%>%
  rename(intercept = b_Intercept, 
         delay = b_delay,
...
model_pred_map <- bind_rows(
  ...
  rename(intercept = Intercept, 
         ...
         interaction = `delay:dietary_breadth`)%>%
  expand_grid(predictor = seq(-1,1, length=100))%>%
  mutate(short_delay = intercept + delay * -1 + main_effect * predictor + interaction * predictor * -1,
         medium_delay = intercept + delay * 0 + main_effect * predictor + interaction * predictor * 0,
         long_delay = intercept + delay * 1 + main_effect * predictor + interaction * predictor * 1)%>%
...
base_model%>% spread_draws(r_species_english[species_english,term])%>%
  ...
  pivot_longer(cols = c(vocal_repertoire, dietary_breadth), names_to = "predictor", values_to = "predictor_value")%>%
  ggplot(aes(x = predictor_value, y = stm_estimate, col = superordinate_group))+
  geom_point(alpha = .5)+
  facet_grid(~predictor, scales = "free_x")+
  geom_smooth(method = lm, se = F, lty = 2, col = "grey")+
  stat_cor(method = "pearson", inherit.aes = F, aes(x = predictor_value, y = stm_estimate), r.accuracy = 0.01, cor.coef.name = "r")+
  theme_few()
