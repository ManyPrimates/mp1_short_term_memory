...
# Read model
m <- readRDS("../saves/base_model.rds")
...
plot_model_pred <- function(d, m, t) {
	...
	td <- as.treedata(tt)
	plot <- ggtree(td, layout="circular", size=2, aes(color=performance)) +
	...
	for(i in 1:length(groups)) {
		...
		plot <- plot + geom_hilight(node=mrca, fill=colours[i], alpha=0.25) + geom_tree()
	}
	plot <- plot + geom_tiplab2(aes(label=species_latin), colour="black", size=6) +
		theme(legend.position = "none")
	return(plot)
}
...
# Composite
predictor_composite <- ggarrange(vocrep, diet, hr, gs,
	labels=c("Vocal repertoire", "Dietary breadth", "Home range", "Group size"))
whole <- ggarrange(model, predictor_composite,
		   labels=c("Predicted STM performance", ""))
ggsave("../figures/phylo_signal.png")
```
